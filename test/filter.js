...
const filter = (obj,ing,available)=>{
 
  const drink = obj.drink_ingredient.reduce((acc,cur)=>{
     return(cur.ingredients_id_list.includes(ing)?[...acc,cur]:acc)
  },[]
  );

  const ings = drink.reduce((accs,curs)=>{
    curs.ingredients_id_list.forEach(it=>{
      (accs.includes(it))?null:(available.includes(it)?accs=[...accs,it]:null);
    })
    return accs; 
   },[])
   
   let index = ings.indexOf(ing);
   ings.splice(index,1);
   console.log(ings);

   result['drink_ingredient']=[...drink];
   result['available_ingredient_list']=[...ings];
...
filter(obj,2,obj.available_ingredient_list);
filter(result,294,result.available_ingredient_list);
